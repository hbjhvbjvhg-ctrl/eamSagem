...
import { Intervention, Priorite, Statut } from '../types/Intervention';
    ...
    <Layout>
      <div className="user-management">
        <div className="user-management-header">
          <div className="header-title"><h1>🛠️ Interventions</h1></div>
            ...
            <tbody>
              {filtered.map(w => (
                <tr key={w.id}>
                  ...
                  <td>{w.priorité}</td>
                  <td>{w.statut}</td>
                  <td>{new Date(w.dateCreation as string).toLocaleDateString()}</td>
                  <td className="actions-cell">
                    ...
                    <button className="action-button" onClick={async () => { if (!window.confirm('Supprimer cette intervention ?')) return; await interventionService.remove(w.id); await load(); }}>🗑️</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {isModalOpen && (
          <div className="modal-overlay" onClick={() => !submitting && setIsModalOpen(false)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header"><h3>{isEditMode ? 'Modifier une intervention' : 'Ajouter une intervention'}</h3></div>
              <div className="modal-body">
                ...
                <div className="form-grid">
                  ...
                  <label>Date de création<input className="input" type="date" value={form.dateCreation ? new Date(form.dateCreation as string).toISOString().slice(0,10) : ''} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, dateCreation: e.target.value })} /></label>
                </div>
              </div>
                ...
                <button className="primary-button" disabled={submitting || !form.titre || !form.description} onClick={async () => {
                  ...
                  try {
                    if (isEditMode && editingId != null) {
                      await interventionService.update({ ...(form as Intervention), id: editingId });
                    } else {
